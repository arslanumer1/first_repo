class Animal:
    def __init__(self, legs, tail=True, name="name"):
        self.legs = legs
        self.tail = True
        self.name = name
...
class UnderGrad(Degree):
    ...
    def __str__(self):
        return (
            f"{self.name} is in {self.num_of_years}th year and in {self.University} "
            f"with {self.num_of_publications} publications"
            f" and under the supervision of {self.advisor}"
        )


Umair = UnderGrad("Umair", "Stefan", 5, 4)
print(Umair)
